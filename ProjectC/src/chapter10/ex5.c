...
int main(int argc, char *argv[])
{
	...
	gtk_signal_connect(GTK_OBJECT(button_ok), "clicked", GTK_SIGNAL_FUNC(callback_ok), entry1);
	gtk_widget_show(button_ok);

	button_cancel=gtk_button_new_with_label("取消");
	gtk_box_pack_start(GTK_BOX(hbox3), button_cancel, TRUE, FALSE,0);
	gtk_signal_connect(GTK_OBJECT(button_cancel), "clicked", GTK_SIGNAL_FUNC(callback_cancel), NULL);
