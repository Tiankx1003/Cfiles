...
static	GtkWidget	*serverwindow , *vscrollbar;	//服务器窗口，用来输出相关提示信息。
...
void about(GtkWidget *widget, gpointer data)
{
	...
	dialog = gnome_dialog_new(_("关于本程序"),_ ( "确定" ) ,NULL ,NULL) ;
	contentlabel=gtk_label_new(_("网络售票模拟系统服务器端"));
...
int main(int argc, char *argv[])
{
	...
	gtk_text_set_editable (GTK_TEXT (serverwindow), FALSE);	//文本信息不可编辑
	gtk_text_set_word_wrap(GTK_TEXT(serverwindow),TRUE );				//自动换行
	gtk_table_attach (GTK_TABLE (table), serverwindow, 0, 1, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL,
