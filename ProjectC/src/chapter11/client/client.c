...
static	GtkWidget	*clientwindow , *vscrollbar;	//服务器窗口，用来输出相关提示信息。
...
void buyticket(GtkWidget *widget, gpointer data)
{
	...
	gtk_signal_connect(GTK_OBJECT(dialog),"clicked" ,GTK_SIGNAL_FUNC(button_buyticket) ,&st_flight) ; //设定“确定”“取消”按钮的回调函数
...
void inquireone()
{
	...
	gtk_signal_connect(GTK_OBJECT(dialog),"clicked" ,GTK_SIGNAL_FUNC(button_inquireone) ,flight_entry) ; //设定“确定”“取消”按钮的回调函数
	gtk_window_set_modal (GTK_WINDOW (dialog), TRUE);
	gtk_widget_show (dialog);
	gnome_dialog_set_parent(GNOME_DIALOG( dialog),GTK_WINDOW(app));	//设定对话框的父窗口为主程序窗口
}
...
void about(GtkWidget *widget, gpointer data)
{
	...
	dialog = gnome_dialog_new(_("关于本程序"),GNOME_STOCK_BUTTON_OK,NULL ,NULL) ;
	contentlabel=gtk_label_new(_("网络售票模拟系统客户端"));
	versionlabel=gtk_label_new(_("版本：0.1"));
	copyrightlabel=gtk_label_new(_("Copyright 2009"));
	authorlabel=gtk_label_new(_("lxy"));
...
int main(int argc, char *argv[])
{
	...
	gtk_text_set_editable (GTK_TEXT (clientwindow), FALSE);	//文本信息不可编辑
	gtk_text_set_word_wrap(GTK_TEXT(clientwindow),TRUE );				//自动换行
	gtk_table_attach (GTK_TABLE (table), clientwindow, 0, 1, 0, 1, GTK_EXPAND | GTK_SHRINK | GTK_FILL,
